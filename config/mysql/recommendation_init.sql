...
-- ========================================
-- 17. 推荐请求日志表 (recommendation_request_logs)
-- 记录推荐请求，用于分析和调优
-- ========================================
DROP TABLE IF EXISTS `recommendation_request_logs`;
CREATE TABLE `recommendation_request_logs` (
    `id` BIGINT NOT NULL AUTO_INCREMENT,
    `request_id` VARCHAR(64) NOT NULL COMMENT '请求ID',
    `user_id` BIGINT NOT NULL COMMENT '用户ID',
    `request_type` VARCHAR(30) DEFAULT 'feed' COMMENT '请求类型: feed/related/search',
    `recall_stats` JSON DEFAULT NULL COMMENT '召回统计 {"hot":10, "cf":20, "content":15}',
    `candidate_count` INT DEFAULT 0 COMMENT '候选视频数',
    ...
    `group_id` BIGINT DEFAULT NULL COMMENT '实验分组ID',
    ...
    KEY `idx_experiment_id` (`experiment_id`),
    KEY `idx_created_at` (`created_at`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='推荐请求日志表';

-- ========================================
-- 18. 推荐模型版本表 (recommendation_model_versions)
-- 管理推荐模型版本
-- ========================================
...
CREATE TABLE `recommendation_model_versions` (
    ...
    `model_type` VARCHAR(30) NOT NULL COMMENT '模型类型: ranking/recall/embedding',
    `model_path` VARCHAR(512) DEFAULT NULL COMMENT '模型文件路径',
    `config` JSON DEFAULT NULL COMMENT '模型配置',
    `metrics` JSON DEFAULT NULL COMMENT '模型指标',
    `status` TINYINT DEFAULT 0 COMMENT '状态 0:offline 1:online 2:deprecated',
    ...
    KEY `idx_status` (`status`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='推荐模型版本表';
