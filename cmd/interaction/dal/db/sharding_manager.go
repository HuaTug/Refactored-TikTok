...
type ShardingManager struct {
	...
	indexManager *GlobalIndexManager   // 全局索引管理器
	mu           sync.RWMutex
}
...
func NewShardingManager(config *ShardingConfig) (*ShardingManager, error) {
	...
	if config.EnableIndex {
		manager.indexManager = NewGlobalIndexManager(manager)
		hlog.Info("Global index manager initialized")
	}
...
func (sm *ShardingManager) initMasterConnections() error {
	for i, dsn := range sm.config.MasterDSNs {
		...
		sqlDB, err := db.DB()
		if err != nil {
			return fmt.Errorf("failed to get sql.DB for master database %d: %w", i, err)
		}
...
func (sm *ShardingManager) initSlaveConnections() error {
	...
	for i, slaveDSNList := range sm.config.SlaveDSNs {
		...
		for j, dsn := range slaveDSNList {
			...
			sqlDB, err := db.DB()
			if err != nil {
				hlog.Warnf("Failed to get sql.DB for slave database %d-%d: %v", i, j, err)
				continue
			}
...
// ExecuteInShardWithIndex 使用全局索引执行查询
func (sm *ShardingManager) ExecuteInShardWithIndex(ctx context.Context, indexKey string, indexValue int64, useWriteDB bool, fn func(db *gorm.DB, tableName string) error) error {
	if sm.indexManager == nil {
		return fmt.Errorf("global index manager not initialized")
	}

	// 从索引中获取分片信息
	shardInfo, err := sm.indexManager.GetShardInfo(ctx, indexKey, indexValue)
	if err != nil {
		return fmt.Errorf("failed to get shard info from index: %w", err)
	}

	dbKey := fmt.Sprintf("db_%d", shardInfo.DBIndex)
	tableName := fmt.Sprintf("comments_%d", shardInfo.TableIndex)
	...
	for dbKey := range sm.masterDBs {
		for tableIdx := 0; tableIdx < sm.config.TableCount; tableIdx++ {
			...
			go func(key string, idx int) {
				defer wg.Done()
				tableName := fmt.Sprintf("comments_%d", idx)
...
// GetIndexManager 获取全局索引管理器
func (sm *ShardingManager) GetIndexManager() *GlobalIndexManager {
...
func (sm *ShardingManager) HealthCheck(ctx context.Context) error {
	...
	for dbKey, db := range sm.masterDBs {
		if sqlDB, err := db.DB(); err != nil {
			return fmt.Errorf("failed to get sql.DB for master %s: %w", dbKey, err)
		} else if err := sqlDB.PingContext(ctx); err != nil {
			return fmt.Errorf("failed to ping master database %s: %w", dbKey, err)
		}
	}
	...
	for dbKey, slaves := range sm.slaveDBs {
		for i, db := range slaves {
			if sqlDB, err := db.DB(); err != nil {
				hlog.Warnf("Failed to get sql.DB for slave %s-%d: %v", dbKey, i, err)
			} else if err := sqlDB.PingContext(ctx); err != nil {
				hlog.Warnf("Failed to ping slave database %s-%d: %v", dbKey, i, err)
			}
		}
	}
...
// GetShardInfo 获取指定shardKey的分片信息
